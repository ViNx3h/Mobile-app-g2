...
export default function SignIn() {
    ...
    const handleLogin = async () => {
        try {
            ...
            const foundUser = userList.find(user => user.username.trim().toLowerCase() === username.trim().toLowerCase());
            if (!foundUser) {
    ...
    return (
        <View className="flex-1 justify-center items-center p-5 bg-gray-900">
            <Text className="text-2xl font-bold text-white mb-5">Đăng Nhập</Text>
            ...
            <TouchableOpacity className="w-full h-12 bg-red-500 rounded-lg justify-center items-center mt-2" onPress={handleLogin}>
